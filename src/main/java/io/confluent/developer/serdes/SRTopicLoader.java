    ...
    public static void main(String[] args) throws IOException {
     ...
     public static void runProducer() throws IOException {
        ...
        try(Admin adminClient = Admin.create(properties);
            ...
            List <String> orderIdList = Arrays.asList("orderNumber-1001",
                                               "orderNumber-5000",
                                               "orderNumber-999",
                                               "orderNumber-3330",
                                               "bogus-1",
            ...
            List<ProductOrder> productOrders = orderIdList.stream().map(orderId-> productOrderBuilder.setOrderId(orderId)
                    .setProduct("HDTV-2333")
                    .setUserId("user-" + random.nextInt(100))
                    .setTime(Instant.now().toEpochMilli())
                    .build()).collect(Collectors.toList());
            List<ProducerRecord<String, ProductOrder>> producerRecords = productOrders.stream().map(r -> new ProducerRecord<>(inputTopic,"order-key", r)).collect(Collectors.toList());
